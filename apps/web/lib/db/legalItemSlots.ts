...
/**
 * スロット1件を上書きする（project_id, slot_key の組で upsert）。
...
export async function setLegalItemSlot(
  ...
  const storedValue = status === "filled" ? value : null;
  if (status === "filled" && (!storedValue || storedValue.trim() === "")) {
  ...
  const { data, error } = await getSupabaseClient()
    ...
    .single();
  if (error) throw error;
  return toLegalItemSlot(data as LegalItemSlotRow);
}
