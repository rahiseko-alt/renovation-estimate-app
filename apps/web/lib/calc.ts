...
/**
 * 採用した下請の原価単価を、施主に出す見積の単価に直すときの掛率。
 *
 * **1（原価そのまま）にしている。** `docs/design.md` は掛率を未決としており、
 * 元請の取り分は諸経費率（案件ごとに人が入れる）が受け持つ。ここで 1 以外を
 * 既定にすると、諸経費と二重に乗る。
 *
 * 明細ごとに掛率を持たせる（旧 quote_requests の markupRate のような）設計に
 * するかどうかは未決。決まるまで、採用の売価はこの1箇所だけが決める
 * （AGENTS.md「結合を増やさない」1：同じ値を2箇所以上に書かない）。
 */
export const ADOPTED_MARKUP_RATE = 1;
