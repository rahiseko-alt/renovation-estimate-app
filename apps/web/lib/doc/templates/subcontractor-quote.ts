 ...
 */
/**
 * 上の注意文の**出典表示**。原文のまま載せる以上、これを外さない。
 *
 * 国土交通省のサイトのコンテンツは「公共データ利用規約（第1.0版）」に沿って
 * 利用でき、**条件は出典の記載**である（編集・加工したときはその旨も別に書く。
 * ここは原文のままなので加工の表示は要らない）。
 * - 利用ルール: https://www.mlit.go.jp/link.html
 * - 規約原文: https://www.digital.go.jp/resources/open_data/public_data_license_v1.0
 *
 * **画面に出す**。コメントに書くだけでは、配布物（この文が印字される書類）に
 * 出典が付かない（2026-08-07 のレビュー指摘）。
 */
export const SUBCONTRACTOR_QUOTE_LEGAL_NOTICE_SOURCE =
  "出典：「【専門工事業者向け】建設工事の見積書様式例」（国土交通省）https://roumuhi.mlit.go.jp/mlit-files/20260316_06.pdf";
