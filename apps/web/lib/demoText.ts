...
export const DEMO_ENTRY_TEXT = {
  start: "デモを触ってみる",
  description: "登録は要りません。3回押すと見積書ができます。",
  /** ログイン導線は残す。デモを始めた人と、実際に使う人は別。 */
  loginNote: "アカウントをお持ちの方",
  /**
   * **既にデモ中の人にも、トップから戻る道を出す。**
   * デモの有効期間は6時間ある。一度見せたあとトップに戻ると、
   * ログイン済み扱いになって入口が消え、**商談で見せ直せなかった**
   * （`docs/failures.md` 2026-08-06）。同じ入口（/demo/start）を押すと
   * 自分のデモへ戻る（中身が古ければ作り直される）。
   */
  resume: "デモの続きへ",
  resumeDescription: "さっきのデモの最初（写真の画面）に戻ります。",
} as const;
