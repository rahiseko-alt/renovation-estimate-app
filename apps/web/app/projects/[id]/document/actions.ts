...
export async function sendFromDocumentAction(
  ...
  //
  // **このとき、打ち直した期限は保存しない**（意図的。2026-08-07 のレビュー指摘への回答）。
  // 依頼は既に下請の手元にあり、その書類に期限が印字されている。こちら側の値だけを
  // 書き換えると、**下請が見ている期限と元請が見ている期限が食い違う**。
  // 送った後の期限を動かせるようにするかどうかは、動かしたことを下請へどう伝えるかと
  // セットで決まる業務の判断で、実装側で決めてよい領域ではない
  // （AGENTS.md「業務ドメインを決めるときの作法」）。**利用者の指示が出るまで動かさない。**
