 ...
 * 残さない**。識別子は普段そのまま使い続ける（新しく発行すると、前の案件が期限まで
 * 残ってしまう。`app/demo/start/route.ts` の①の注記と同じ理由）。
 *
 * **例外が1つある。写真の実体を消せなかったときは、識別子が新しいものに変わる。**
 * 前の識別子の行（実体を消すための `storage_path`）を残しておかないと、
 * 残った実体を後から消す手がかりが無くなるため。前の識別子ぶんは期限切れの
 * 掃除が引き取るので、**利用者から見た「やり直し」はどちらの場合も成功する。**
 *
