...
describe("下請からの回答", () => {
  ...
  it("依頼されていない明細の単価は入れられない（tokenを1つ持っていても範囲外は書けない）", async () => {
    ...
    ).rejects.toThrow("依頼されていない明細が含まれています。");
  });
  ...
  it("回答すると依頼の状態が「回答あり」に進む（元請の比較表がこれを見る）", async () => {
    ...
    // 「回答があった」ことは依頼側の状態で表す（比較表がこの列を見る）。
    // 回答行の有無だけを見ると、状態更新が壊れても気付けない。
    const updated = await getQuoteGroupRequestForOwner(request.id, OWNER_A);
