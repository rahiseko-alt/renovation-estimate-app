...
describe("lineAmount", () => {
  ...
  it("浮動小数点の誤差で1円ずれない（真値より上に出る誤差）", () => {
    // 1.1 * 90 は素の浮動小数点演算だと 99.00000000000001 になるが、
    // 数量を1/1000単位の整数に丸めてから BigInt で正確に計算するので、
    // この手のノイズはそもそも発生しない。
    expect(lineAmount(line({ quantity: 1.1, unitPrice: 90 }))).toBe(99);
  });

  it("浮動小数点の誤差で1円ずれない（真値より下に出る誤差）", () => {
    // 1.4 * 1400 は素の浮動小数点演算だと 1959.9999999999998 になり、
    // そのまま切り捨てると1円少ない。BigInt の正確な演算では起きない。
  ...
  it("数量は1/1000単位に丸める（それより細かい桁は仕様として丸められる）", () => {
    // 1.9999996 は 1/1000 単位に丸めると 2.000 になる。これは「浮動小数点の
    // ノイズを誤って動かした」のではなく、数量の精度を1/1000単位までと
    // 決めている設計どおりの丸め。㎡・m・人工の実務でこれより細かい桁の
    // 数量が要ることは無い。
    expect(lineAmount(line({ quantity: 1.9999996, unitPrice: 1 }))).toBe(2);
    // 1/1000単位に収まる値は丸められず、そのまま切り捨てられる
    expect(lineAmount(line({ quantity: 1.999, unitPrice: 1 }))).toBe(1);
  });

  it("極端に大きい数量は例外を投げる（実務でありえない値を弾く）", () => {
    // 数量が極端に大きいと、1/1000単位への丸めが浮動小数点の精度限界に
    // 近づき、逆に不正確になりうる。実務であり得ない大きさは上限で弾く。
    expect(() =>
      lineAmount(line({ quantity: 999_999_999_999.999, unitPrice: 1 })),
    ).toThrow(/数量が大きすぎます/);
  });
